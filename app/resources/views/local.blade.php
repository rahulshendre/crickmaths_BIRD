...
<div class="container">
                    ...
                    <div class="question content p-3 border-bottom" id="quiz score">
                       <!--   <button class="btn btn-primary" id="next">Next</button>
  <button class="btn btn-info" id="prev">Prev</button -->
<p id="total-score-display">Total Score: 0</p>
                    </div>
                    <div class="timeline" id="timeline-container"></div>
                                <div class="d-flex flex-row justify-content-between align-items-center p-3 ">
                                    <input type="hidden" name="user_id" value="3">
...
function escapeRegExp(string) {
  return string.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    ...
    'blanks1': 'replacement1',
    'blanks2': 'replacement2',
      'blanks3': 'replacement4',
      'blanks4': 'replacement5',
      'blanks5': 'replacement6',
'Team':'replacement7',
  };
...
function displayNext() {
  quiz.fadeOut(function () {
    ...
    if (questionCounter < questions.length) {
      ...
      // Display the current score along with the question only if the answer is correct
...
// Helper function to calculate the current score and update the total score
function calculateCurrentScore(isCorrect) {
  // Calculate the current score for the current question only if the answer is correct
...
// Call the function to generate timeline parts when the page loads
$(document).ready(function () {
  generateTimelineParts();
});


// Click handler for the 'View Detailed Scores' button
$('#view-scores').on('click', function (event) {
  event.preventDefault();
...
var currentPart = 1;
 ...
 // Click handler for the 'Start Over' button
      ...
      // Function to replace words in a sentence using dynamic content from the backend
// Function to replace placeholders in a sentence with dynamic content
function replaceWords(sentence, replacements, callback) {
    // Make an AJAX request to fetch dynamic words from the backend
    $.ajax({
        type: "GET",
        ...
        success: function (data) {
            ...
            // Callback with the updated sentence
            callback(sentence);
        },
        error: function (error) {
            console.error("Error fetching dynamic words:", error);
            
            // Callback with the original sentence if an error occurs
            callback(sentence);
        }
    });
}
...
function escapeRegExp(string) {
    ...
    $.ajax({
        ...
        url: '{{config("app.baseURL")}}/getDynamicWords?match_id=' + "{{$match_id}}",
        success: function (data) {
            // Sort dynamic replacements based on keys to ensure correct order
...
function calculateCurrentScore(isCorrect) {
 ...
 var addScore=numCorrect*10;
  ...
  scoreboardHTML += '<h1 class="correct">'+addScore+'</h1>';
 scoreboardHTML += '<div class="scoreButtons"><button id="button1" class="quiz-button button1" >Button 1</button></div>';
   ...
   $(document).on('click', '#view-scores', function (event) {
    ...
    initializeDataTable(userAttempts);
      ...
      function animateFeedback(isCorrect) {
        ...
        var feedbackImage = $('<img>', {
          src: isCorrect ? 'assets/images/six.webp' : 'assets/img/wickt.gif',
          alt: isCorrect ? 'Correct!' : 'Incorrect!',
          class:'umpire'
        });
...
function openDropdown() {
 ...
 <div class="row mb-3">
                        ...
                        <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin="">
                          ...
                          <?php
$teamName = $team['teamFullName']; // Replace with the desired cricket team name

// Using the CountryFlags API
$apiUrl = "https://restcountries.com/v2/name/" . urlencode($teamName) . "?fields=flags";
...
// Check if the response contains the flag information
if (isset($data[0]['flags']['svg'])) {
    $flagUrl = $data[0]['flags']['svg'];

    // Output the flag image
    echo "<img src=\"$flagUrl\" alt=\"$teamName Flag\">";
} else {
    echo "Flag not found for $teamName";
}
?>


 Team0 scored tscore0 runs in 25 overs. Calculate the run rate per over for Team0.