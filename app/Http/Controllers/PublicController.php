...
{
    /**
     * Convert match ID (UUID or numeric) to integer for database storage
     */
    private function convertMatchIdToInteger($matchId)
    {
        // If it's already numeric, return as is (but ensure it fits in INT range)
        if (is_numeric($matchId)) {
            $intValue = (int) $matchId;
            // Ensure it fits in signed INT range (-2,147,483,648 to 2,147,483,647)
            return abs($intValue) % 2147483647;
        }
        
        // Convert UUID to integer using hash and ensure it fits in signed INT range
        $hash = abs(crc32($matchId));
        // Use modulo to ensure it fits in signed INT range
        return $hash % 2147483647;
    }

    public function getliveScore(Request $request){
        $matchID = $this->convertMatchIdToInteger($request->match_id);
...
$matchID = $this->convertMatchIdToInteger($req->selectedValue);

// Use CricketApiService instead of direct API call
$cricketService = new CricketApiService();
$finalcall = $cricketService->getScorecard($req->selectedValue); // Keep original UUID for API call
...
if (empty($finalcall)) {
 ...
 $matchLive=Live::where('match_id',$matchID)->get();
         $live_id="";  
$match_id=$matchID;  // Use converted integer
...
$match_id = $this->convertMatchIdToInteger($request->match_id);
$result=Livematch::where('match_id',$match_id)->get();
$resultLive=Live::where('match_id',$match_id)->first();
...
if (!empty($finalcall) && isset($finalcall['data']['featurematch'])) {
    foreach ($finalcall['data']['featurematch'] as $matchData) {
            if (isset($matchData['matchType']) && $matchData['matchType'] != "Test") {
                $match_id = $this->convertMatchIdToInteger($matchData['matchID']);
...
$selectedValue = $this->convertMatchIdToInteger($request->selectedValue);
...
$scorecardData = $cricketService->getScorecard($matchData['matchID']);
